...
export function makeAge(n: number): Age {
  if (!Number.isInteger(n)) throw new Error(`Age must be an integer, got: ${n}`)
  if (n < 0 || n > 130) throw new Error(`Invalid age: ${n}. Must be between 0 and 130.`)
  return n as Age
}

export function makeRoomNumber(n: number): RoomNumber {
  if (!Number.isInteger(n)) throw new Error(`Room number must be an integer, got: ${n}`)
...
export function makeHour(n: number): Hour {
  if (!Number.isInteger(n)) throw new Error(`Hour must be an integer, got: ${n}`)
  if (n < 0 || n > 23) throw new Error(`Invalid hour: ${n}. Must be between 0 and 23.`)
  return n as Hour
}
...
export type PatientEvent =
  ...
  | { type: "PatientRegistered"; payload: { id: PatientId; name: PatientName; age: Age; roomNumber: RoomNumber } }
...
export function dischargePatient(patient: ObservablePatient): ObservablePatient {
  if (patient.status !== "checked-in") {
    throw new Error(`Patient must be checked in before being discharged. Current status: ${patient.status}`)
  }
 ...
 */
export function updateRoomNumber(patient: ObservablePatient, newRoom: number): ObservablePatient {
